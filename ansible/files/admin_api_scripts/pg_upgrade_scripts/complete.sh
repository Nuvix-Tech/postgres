...
function execute_patches {
    ...
    if [ "$PG_NET_ENABLED" = "t" ]; then
        ...
        GRANT USAGE ON SCHEMA net TO nuvix_functions_admin, postgres, anon, authenticated, service_role;
...
EOF
    ...
    if [ "$HAS_PG_CRON_OWNED_BY_POSTGRES" = "t" ]; then
        ...
        update cron.job set username = 'postgres' where username = 'nuvix_admin';
        commit;
EOF
        )
...
function start_vacuum_analyze {
    ...
    vacuumdb --all --analyze-in-stages -U nuvix_admin -h localhost -p 5432
