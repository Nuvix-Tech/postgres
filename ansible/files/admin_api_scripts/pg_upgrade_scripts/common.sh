...
function run_sql {
    psql -h localhost -U nuvix_admin -d postgres "$@"
}
...
EOSQL
    ...
    retry 8 pg_isready -h localhost -U nuvix_admin
...
revoke nuvix_admin from authenticator;
...
declare
  ...
  nuvix_admin_rolpassword text := (select rolpassword from pg_authid where rolname = 'nuvix_admin');
  role_settings jsonb[] := (
    ...
    where a.rolname in ('postgres', 'nuvix_admin')
  );
...
begin
  ...
  alter role postgres rename to nuvix_admin_;
  alter role nuvix_admin rename to postgres;
  alter role nuvix_admin_ rename to nuvix_admin;
  ...
  loop
    ...
    execute(format(
      ...
      case
        when rec.roleid = 'postgres'::regrole then 'nuvix_admin'
        when rec.roleid = 'nuvix_admin'::regrole then 'postgres'
        else rec.roleid::regrole
      end,
      case
        when rec.member = 'postgres'::regrole then 'nuvix_admin'
        when rec.member = 'nuvix_admin'::regrole then 'postgres'
      ...
      case
        when rec.grantor = 'postgres'::regrole then 'nuvix_admin'
        when rec.grantor = 'nuvix_admin'::regrole then 'postgres'
  ...
  execute(format('alter role nuvix_admin password %L;', nuvix_admin_rolpassword));

  -- role settings
  foreach obj in array role_settings
  loop
    execute(format('alter role %I %s reset all',
                   case when obj->>'role' = 'postgres' then 'nuvix_admin' else 'postgres' end,
  ...
  reassign owned by postgres to nuvix_admin;
  ...
  loop
    execute(format('drop user mapping for %I server %I', case when obj->>'role' = 'postgres' then 'nuvix_admin' else 'postgres' end, obj->>'server'));
  end loop;
  foreach obj in array user_mappings
  loop
  ...
  loop
    ...
    loop
      if obj->>'role' in ('postgres', 'nuvix_admin') or rec.grantee::regrole in ('postgres', 'nuvix_admin') then
        execute(format('alter default privileges for role %I %s revoke %s on %s from %s'
                     , case when obj->>'role' = 'postgres' then 'nuvix_admin'
                            when obj->>'role' = 'nuvix_admin' then 'postgres'
                            else obj->>'role'
                       end
                     ...
                     , case when rec.grantee = 'postgres'::regrole then 'nuvix_admin'
                            when rec.grantee = 'nuvix_admin'::regrole then 'postgres'
  ...
  loop
    ...
    for rec in
      select grantor, grantee, privilege_type, is_grantable
      from aclexplode((obj->>'acl')::aclitem[])
      where grantee::regrole in ('postgres', 'nuvix_admin')
    loop
      execute(format('revoke %s on schema %s from %I', rec.privilege_type, (obj->>'oid')::regnamespace, case when rec.grantee = 'postgres'::regrole then 'nuvix_admin' else 'postgres' end));
    end loop;
  end loop;
  foreach obj in array schemas
  loop
  ...
  loop
    ...
    for rec in
      ...
      where grantee::regrole in ('postgres', 'nuvix_admin')
    loop
      execute(format('revoke %s on type %s from %I', rec.privilege_type, (obj->>'oid')::regtype, case when rec.grantee = 'postgres'::regrole then 'nuvix_admin' else 'postgres' end));
    end loop;
  end loop;
  foreach obj in array types
  loop
  ...
  loop
    ...
    loop
      execute(format('revoke %s on %s %s(%s) from %I'
          ...
          , (obj->>'oid')::regproc
          , pg_get_function_identity_arguments((obj->>'oid')::regproc)
          , case when rec.grantee = 'postgres'::regrole then 'nuvix_admin' else 'postgres' end
          ));
    end loop;
  end loop;
  foreach obj in array functions
  ...
  loop
    ...
    for rec in
      ...
      where grantee::regrole in ('postgres', 'nuvix_admin')
    loop
      execute(format('revoke %s on table %s from %I', rec.privilege_type, (obj->>'oid')::oid::regclass, case when rec.grantee = 'postgres'::regrole then 'nuvix_admin' else 'postgres' end));
    end loop;
  end loop;
  foreach obj in array relations
  loop
...
set session authorization nuvix_admin;
