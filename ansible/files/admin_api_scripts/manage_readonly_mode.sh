...
function set_mode {
    MODE=$1
    psql -h localhost -U nuvix_admin -d postgres -c "ALTER SYSTEM SET default_transaction_read_only to ${MODE};"
    psql -h localhost -U nuvix_admin -d postgres -c "SELECT pg_reload_conf();"
}
...
EOF
    ...
    RESULT=$(psql -h localhost -U nuvix_admin -d postgres -At -c "$COMMAND")
    echo -n "$RESULT"
}
