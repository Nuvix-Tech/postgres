      ...
      let
        ...
        makeCheckHarness = pgpkg:
          ...
          pkgs.runCommand "postgres-${pgpkg.version}-check-harness"
            ...
            initdb --locale=C --username=nuvix_admin -D "$PGTAP_CLUSTER"
            ...
            createdb -p 5435 -h localhost --username=nuvix_admin testing
            if ! psql -p 5435 -h localhost --username=nuvix_admin -d testing -v ON_ERROR_STOP=1 -Xaf ${./nix/tests/prime.sql}; then
            ...
            for t in $(printf "%s\n" ${builtins.concatStringsSep " " sortedTestList}); do
              psql -p 5435 -h localhost --username=nuvix_admin -d testing -f "${./nix/tests/sql}/$t.sql" || true
            done
            rm -rf "$SORTED_DIR"
            pg_ctl -D "$PGTAP_CLUSTER" stop
            rm -rf $PGTAP_CLUSTER
            ...
            for i in {1..60}; do
                if pg_isready -h localhost -p 5435 -U nuvix_admin -q; then
                    echo "PostgreSQL is ready"
                    break
                fi
                sleep 1
            ...
            if ! psql -p 5435 -h localhost --no-password --username=nuvix_admin -d postgres -v ON_ERROR_STOP=1 -Xaf ${./nix/tests/prime.sql}; then
            ...
            pg_prove -p 5435 -U nuvix_admin -h localhost -d postgres -v ${./migrations/tests}/test.sql
